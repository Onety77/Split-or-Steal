...
const MIN_TOKENS = 0; // any balance qualifies — verified on-chain
...
async function verifyHolding(wallet) {
  try {
    ...
    if (tokenAccounts.value.length === 0) return { holds: false, amount: 0 };
...
export default function Queue({ navigate }) {
  ...
  useEffect(() => {
    if (myEntry?.status === "in_duel" && myEntry?.currentDuelId) navigate("duel");
  ...
  const joinQueue = async () => {
    ...
    try {
      const { holds, skipped } = await verifyHolding(profile.wallet);
      if (!holds && !skipped) {
        setJoinError("You need to hold $SOS tokens to join. Your wallet shows 0 tokens.");
        setVerifying(false);
        return;
      }
      setVerifying(false);
      ...
      await setDoc(doc(db, "sos_queue", user.uid), {
        uid: user.uid, username: profile.username, wallet: profile.wallet,
        joinedAt: serverTimestamp(), status: "waiting",
        readyCheckEndsAt: null, currentDuelId: null,
      });
    } catch (e) {
      console.error(e);
  ...
  const estimatedWait = myPosition ? Math.max(0, Math.ceil((myPosition - 2) / 2)) * 5 : null;
      ...
      <div style={{ maxWidth:"var(--max-w)", margin:"0 auto" }}>
        ...
        <div style={{ marginBottom:48 }}>
          ...
          <p style={{ fontFamily:"'Barlow',sans-serif", fontSize:15, color:"var(--muted)", lineHeight:1.7, maxWidth:520 }}>
            Every few minutes, the top two players in the queue are called to duel.
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))", gap:24, alignItems:"start" }}>
          ...
          <div>
                ...
                {!myEntry ? (
                  <div className="card" style={{ border:"1px solid rgba(255,184,0,0.25)" }}>
                    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:16, flexWrap:"wrap", marginBottom:joinError?12:0 }}>
                      <div>
                        ...
                        {verifying && <div style={{ fontFamily:"'Barlow',sans-serif", fontSize:12, color:"var(--muted)", marginTop:4 }}>Checking your $SOS holdings...</div>}
                      </div>
                  ...
                  <div className="card" style={{ border:"1px solid rgba(0,200,83,0.2)", background:"rgba(0,200,83,0.04)" }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:12 }}>
                      <div>
                        <div className="label" style={{ marginBottom:6, color:"var(--green)" }}>{myPosition ? positionLabel(myPosition) : "IN QUEUE"}</div>
                        <div style={{ fontFamily:"'Oswald',sans-serif", fontSize:18, color:"var(--text)" }}>{profile.username}</div>
                        {estimatedWait !== null && estimatedWait > 0 && <div style={{ fontSize:12, color:"var(--dim)", marginTop:4, fontFamily:"'Barlow',sans-serif" }}>~{estimatedWait} min estimated wait</div>}
                        {(myPosition === 1 || myPosition === 2) && <div style={{ fontSize:12, color:"var(--gold)", marginTop:4, fontFamily:"'Barlow',sans-serif" }}>⚡ Your duel is starting soon — stay ready!</div>}
                      </div>
            ...
            <div className="card" style={{ padding:"20px 24px" }}>
              ...
              {[
                ["01","Sign in and click Join Queue. Must hold $SOS tokens in your wallet."],
                ["02","Every few minutes, the top 2 in queue are called to duel."],
                ["03","A full-screen alert fires with sound — 45 seconds to click READY or you are ejected."],
            ...
            {queueList.length === 0 ? (
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                {queueList.map((entry, i) => {
                  ...
                  const isTop = i < 2;
                  const statusIcon = entry.status==="ready" ? "✓" : entry.status==="ready_check" ? "⏳" : entry.status==="in_duel" ? "⚔️" : null;
                  return (
