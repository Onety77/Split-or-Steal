...
function HowItWorksModal({ onClose }) {
  ...
  const outcomes = [
    { votes:["SPLIT","SPLIT"], color:"var(--green)", border:"rgba(0,200,83,0.25)", bg:"rgba(0,200,83,0.06)", bar:"linear-gradient(90deg,#00C853,#69F0AE)", title:"BOTH SPLIT", icon:"🤝", desc:"Trust wins. The pot is split equally. Everyone walks away with something." },
    { votes:["STEAL","SPLIT"], color:"var(--red2)",  border:"rgba(204,32,32,0.25)", bg:"rgba(204,32,32,0.06)", bar:"linear-gradient(90deg,#CC2020,#FF5252)", title:"BETRAYAL",   icon:"🗡️", desc:"One player trusted. One stole. The stealer takes everything. The victim walks away with nothing. Their wallet is on the ledger forever." },
    { votes:["STEAL","STEAL"], color:"#90A4AE",      border:"rgba(96,125,139,0.25)", bg:"rgba(69,90,100,0.08)", bar:"linear-gradient(90deg,#37474F,#607D8B)", title:"BOTH STEAL", icon:"💀", desc:"Neither trusted the other. Both reached for everything and got nothing. The pot carries over and grows for next round." },
  ];
    ...
    <div onClick={e => e.target === e.currentTarget && onClose()} style={{ position:"fixed", inset:0, zIndex:600, background:"rgba(8,6,4,0.93)", display:"flex", alignItems:"center", justifyContent:"center", padding:"20px 16px", animation:"fade-in 0.25s ease", backdropFilter:"blur(8px)", WebkitBackdropFilter:"blur(8px)" }}>
      <div style={{ width:"100%", maxWidth:520, background:"var(--bg2)", border:"1px solid rgba(255,184,0,0.15)", borderRadius:20, overflow:"hidden", animation:"slide-up 0.3s ease", maxHeight:"90vh", overflowY:"auto" }}>
        <div style={{ padding:"22px 26px 18px", borderBottom:"1px solid var(--border)", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div>
            <div className="label" style={{ marginBottom:6 }}>THE GAME</div>
            <h2 style={{ fontFamily:"'Russo One',sans-serif", fontSize:22, letterSpacing:"0.08em", color:"var(--text)", margin:0 }}>HOW IT WORKS</h2>
          </div>
        ...
        <div style={{ padding:"18px 26px", borderBottom:"1px solid var(--border)" }}>
          {[
            ["01","Hold $10+ worth of $SOS to qualify. Wallet verified automatically."],
            ["02","Join the queue. Every 10 minutes, top two players are called."],
            ["03","You get 90 seconds to click READY or you are ejected."],
            ["04","Chat opens with your opponent. Negotiate or stay silent — 3 minutes."],
            ["05","Vote in secret: SPLIT or STEAL. 2 minutes to decide. Opponent cannot see."],
            ["06","Both votes reveal simultaneously. SOL sent on-chain instantly."],
          ].map(([n,t]) => (
              ...
              <div key={o.title} style={{ padding:"14px 16px", background:o.bg, border:"1px solid " + o.border, borderRadius:12, position:"relative", overflow:"hidden" }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:8 }}>
                  <div style={{ display:"flex", gap:5, flexShrink:0 }}>
                    {o.votes.map((v,i) => <Orb key={i} type={v} size={32} animated={false}/>)}
                  </div>
    ...
    <div className="page">
      ...
      <section style={{ position:"relative", minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding: isMobile ? "88px 20px 60px" : "100px 24px 72px", overflow:"hidden", textAlign:"center" }}>
        ...
        <div style={{ position:"relative", zIndex:2, width:"100%", maxWidth:860 }}>
          <p style={{ fontFamily:"'Oswald',sans-serif", fontSize: isMobile ? 9 : 11, fontWeight:500, letterSpacing: isMobile ? 5 : 7, color:"var(--gold)", marginBottom:16, animation:"fade-in 0.7s ease 0.2s both" }}>
            ON-CHAIN · REAL MONEY · REAL BETRAYAL
          </p>
          ...
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:12, animation:"slide-up 0.8s ease 0.9s both" }}>
            <button onClick={() => navigate("queue")} className="btn-gold" style={{ fontSize: isMobile ? 14 : 15, padding: isMobile ? "14px 0" : "15px 44px", width: isMobile ? "100%" : "auto", maxWidth:320 }}>
              JOIN THE QUEUE
            </button>
            <button onClick={() => setShowHowModal(true)} className="btn-outline" style={{ fontSize: isMobile ? 13 : 14, padding: isMobile ? "12px 0" : "13px 40px", width: isMobile ? "100%" : "auto", maxWidth:320 }}>
              HOW IT WORKS
            </button>
          </div>
        </div>
        ...
        {activeDuel ? (
          <div className="card" style={{ border:"1px solid rgba(255,184,0,0.25)", animation:"glow-gold 4s ease-in-out infinite", position:"relative", overflow:"hidden", padding: isMobile ? "18px 14px" : "24px 28px" }}>
            ...
            <div style={{ textAlign:"center", marginBottom:10 }}>
              <div style={{ fontFamily:"'Russo One',sans-serif", fontSize: isMobile ? 18 : 22, color:"var(--gold)" }}>
                ◎ {fmtSOL(activeDuel.amount)} <span style={{ color:"var(--muted)", fontSize:11 }}>at stake</span>
              </div>
            </div>
        ...
        {duels.length === 0 ? (
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {duels.map((d, i) => {
              ...
              return (
                <div key={d.id} style={{ background:"var(--card)", border:"1px solid " + color + "22", borderRadius:12, padding: isMobile ? "14px" : "18px 20px", position:"relative", overflow:"hidden", animation:"slide-up 0.5s ease " + (i*0.05) + "s both" }}>
                  ...
                  <div style={{ marginTop:10, textAlign:"right", fontSize:10, color:"var(--dim)", fontFamily:"'Barlow',sans-serif" }}>{d.timestamp ? timeAgo(d.timestamp.toMillis()) : ""}</div>
                </div>
              );
            })}
          </div>
